...
(define (match-up* population0 rounds-per-match)
  ...
  (population-reset a*)
  ;; -- IN --
  (for ([i (range 0 (- (vector-length a*) 1) 2) #;(in-range 0 (- (vector-length a*) 1) 2)])
...
;; -----------------------------------------------------------------------------
(define (death-birth population0 rate q) ;; TODO ;;#:random (q #false))
  (match-define (cons a* b*) population0)
  (define payoffs
    (for/list  ([x  (in-vector a*)])
      (automaton-payoff x)))
  (let ([substitutes (choose-randomly payoffs rate q)])
    (for ([i rate #;(in-range rate)][p (in-list substitutes)])
